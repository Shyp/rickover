...
func TestWorkerMakesCorrectRequest(t *testing.T) {
	t.Skip("This test needs a database call to update the job to 'success' after it makes a request. Maybe a dummy worker server")
	db.SetUp(t)
	defer db.TearDown(t)
	...
	for {
		select {
		case <-c1:
			test.AssertEquals(t, path, fmt.Sprintf("/v1/jobs/echo/job_%s/work", id.String()))
		...
		case <-time.After(200 * time.Millisecond):
			t.Fatalf("pool did not shut down in 200ms")
		default:
			continue
		}
	}
